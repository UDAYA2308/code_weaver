...
def test_fetch_url_404():
    ...
    with patch("urllib.request.urlopen") as mock_url:
        ...
        mock_url.side_effect = HTTPError("http://example.com", 404, "Not Found", {}, None)
        result = fetch_url.invoke({"url": "http://example.com"})
        assert "Error fetching URL" in result
...
def test_read_file_encoding_error():
    ...
    from src.code_weaver.tools.file_tools import _PROJECT_ROOT

    # Create a temporary file with invalid utf-8 bytes
    with tempfile.NamedTemporaryFile(dir=_PROJECT_ROOT, delete=False) as tmp:
...
def test_read_file_non_existent():
    ...
    result = read_file.invoke({"path": "C:\\this_file_definitely_does_not_exist_12345.txt"})
