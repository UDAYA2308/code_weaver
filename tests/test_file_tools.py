...
from src.code_weaver.tools.file_tools import read_file, write_file, edit_file, list_dir, search
...
def test_edit_file(test_dir):
    path = str(test_dir / "hello.txt")
    result = edit_file.invoke({"path": path, "old_content": "Hello World", "new_content": "Hello Universe"})
...
def test_list_dir(test_dir):
    ...
    with patch("src.code_weaver.tools.file_tools._PROJECT_ROOT", test_dir), \
...
def test_search(test_dir):
    ...
    with patch("src.code_weaver.tools.file_tools._PROJECT_ROOT", test_dir), \
         patch("src.code_weaver.tools.file_tools._GITIGNORE_PATTERNS", ["ignored.txt", ".gitignore"]):
