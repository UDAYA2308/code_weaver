...
def test_path_security_restricted_mode_allowed(tmp_path):
    ...
    with (
        patch(
            "code_weaver.config.config.paths.allowed_paths", [str(test_dir.resolve())]
        ),
        patch("code_weaver.config.config.paths.blocked_paths", []),
    ):
        # Should be allowed
...
def test_path_security_restricted_mode_denied(tmp_path):
    ...
    with (
        patch(
            "code_weaver.config.config.paths.allowed_paths",
            [str(allowed_dir.resolve())],
        ),
        patch("code_weaver.config.config.paths.blocked_paths", []),
    ):
...
def test_path_security_regex_block(tmp_path):
    ...
    with (
        patch("code_weaver.config.config.paths.allowed_paths", []),
        patch("code_weaver.config.config.paths.blocked_paths", [r".*\.env$"]),
    ):
        result = read_file.invoke({"path": str(test_file)})
        assert "Security Error" in result
        assert "explicitly blocked" in result
...
def test_path_security_regex_allow(tmp_path):
    ...
    with (
        patch("code_weaver.config.config.paths.allowed_paths", [r".*\.py$"]),
        patch("code_weaver.config.config.paths.blocked_paths", []),
    ):
        result = read_file.invoke({"path": str(test_file)})
